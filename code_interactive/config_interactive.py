...
class InteractiveConfig:
    ...
    Judge
    -----
    judge_min_turn          : 판정 시작 최소 턴 / min turn to start judging
    judge_max_new_tokens    : Judge 최대 출력 토큰 / judge max output tokens
    judge_sampling          : Judge 생성 전략 / judge generation strategy
    judge_output_format     : 출력 포맷 / output format
    judge_align_threshold   : aligned 임계값 (binary 제외) / alignment threshold
    judge_use_goal_def      : goal_definition 블록 포함 여부 / use goal_def block
    judge_use_workflow      : workflow 블록 포함 여부 / use workflow block

    Conversation
    ------------
    max_turns               : 최대 턴 수 (안전 상한) / max turns (safety ceiling)
    context_window          : 공유 히스토리 슬라이딩 윈도우 / sliding context window
    summarize_every         : 요약 갱신 주기 / summary update interval
    ...
    n_gpu_layers:  int = -1      # -1=전체 레이어 GPU 오프로드 / -1=all layers on GPU
    ...
    # ── Judge ─────────────────────────────────────────────────────────────────
    judge_min_turn:       int   = 3
    judge_max_new_tokens: int   = 40
    ...
    judge_use_goal_def:  bool = True
    judge_use_workflow:  bool = True
